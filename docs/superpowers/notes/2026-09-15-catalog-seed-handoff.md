...
> **업데이트 (같은 날, 이후)**: 이 노트를 쓴 뒤 WebFetch(위키백과·나무위키 교차확인)로 우회해
> 6개 브랜드 전부 검증을 마쳤다. 기아 재검증(오류 2건 수정, 상충 1건 제외), 제네시스/
> KG모빌리티/르노코리아/쉐보레 신규 작성까지 전부 완료. `server/scripts/validateCatalog.js`도
> 작성해 4개 검사 항목 전부 0건 통과 확인함 (커밋: `58606b8`, `a28b063`, `dc5fc84`, `ef1829e`,
> `b51cc47`, `27a6c09`). 아래 "브랜드별 상태" 표와 제네시스 초안 섹션은 **작성 당시 스냅샷이라
> 지금은 대부분 사실이 아니다** — 최신 상태는 `server/db/seed.js`와 `git log`를 직접 확인할 것.
> 이 문서는 "WebSearch 예산 소진 시 WebFetch로 우회 가능하다"는 교훈과 세대 구분 규칙을
> 남기기 위해 보존한다.
